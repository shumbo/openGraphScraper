  ...
  if (!options.onlyGetOpenGraphInfo) {
    ...
    if (!ogObject.ogImage && ogImageFallback) {
      ...
      if(fallbackImages.length > 0){
        if(options.allMedia) {
          ogObject.ogImage = fallbackImages
        } else {
          ogObject.ogImage = fallbackImages[0]
        }
      }
    }
  }

  // remove ogObject.ogImage if
  // 1. ogImage is an array and is empty
  // 2. ogImage is an object an has no key
  if ((Array.isArray(ogObject.ogImage) && ogObject.ogImage.length <= 0) || (ogObject.ogImage && Object.keys(ogObject.ogImage).length <= 0)) {
    delete ogObject.ogImage;
  }
